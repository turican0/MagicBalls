...
bool LoadMusicTrack(uint8_t* fullMusicData, type_v8 headerx[4], uint8_t drivernumber) {
    if (headerx[drivernumber].dword_4 == -1) return false;

    if (musicData_E3810) free(musicData_E3810);
    if (musicHeader_E3808) free((uint8_t*)musicHeader_E3808);

    musicData_E3810 = (uint8_t*)malloc(headerx[drivernumber].dword_12);
    if (!musicData_E3810) return false;

    musicHeader_E3808 = (type_E3808_music_header*)malloc(sizeof(type_E3808_music_header));
    if (!musicHeader_E3808) { free(musicData_E3810); return false; }
    ...
    if (!shadow_str_E3808_music_header) { free(musicData_E3810); free((uint8_t*)musicHeader_E3808); return false; }

    uint32_t musicDataOffset = headerx[drivernumber].dword_4;
    uint32_t musicDataSize = headerx[drivernumber].dword_12;
    memcpy(musicData_E3810, fullMusicData + musicDataOffset, musicDataSize);

    uint32_t musicHeaderOffset = headerx[drivernumber].dword_0;
    uint32_t musicHeaderSize = headerx[drivernumber].sizeBytes_8;
    memcpy(shadow_str_E3808_music_header, fullMusicData + musicHeaderOffset, 8);
    memcpy(&(shadow_str_E3808_music_header->str_8), fullMusicData + musicHeaderOffset + 8, musicHeaderSize - 8);
    ...
    free((uint8_t*)shadow_str_E3808_music_header);
    ...
    for (int i = 1; i <= m_iNumberOfTracks; i++) {
        ...
        track[i - 1].end = track[i - 1].begin + musicHeader_E3808->str_8.track_10[i - 1].xmiSize_8;
    }
...
int main() {
    ...
    if (channellplus <= driverarray_mem[finaldrivernumber]) {
        ...
        if (LoadMusicTrack(fullMusicData, headerx, finaldrivernumber))
            loaded = true;
    }
